...
namespace WoWonder.Activities.NativePost.Post
{
    public class AdapterBind
    {
        ...
        public void BottomPostPartBind(AdapterHolders.PostBottomSectionViewHolder holder, AdapterModelsClass item, bool visible = true)
        {
            try
            {
                    ...
                    switch (string.IsNullOrEmpty(item.InfoUserModel.UserData.Birthday))
                    {
                        case false when item.InfoUserModel.UserData.BirthPrivacy != "2" && item.InfoUserModel.UserData.Birthday != "00-00-0000" && item.InfoUserModel.UserData.Birthday != "0000-00-00":
                            try
                            {
                                ...
                                //string newFormat = String.Format("%.2d", date.Month) + "/" + String.Format("%.2d", date.Day) + "/" + date.Year;
                                //string newFormat = date.Day + "/" + date.Month + "/" + date.Year;
                                string newFormat = String.Format("%.2d", date.Month) + "/" + String.Format("%.2d", date.Day);
                            ...
                            catch
                            {
                                //Methods.DisplayReportResultTrack(e);
                                string newFormat = "";
                                var date = item.InfoUserModel.UserData.Birthday.Split(new char[] { '-', '/' });
                                if (date.Length is > 0)
                                {
                                    if (date[0].Length is > 2)
                                    {
                                        newFormat = date[1] + "/" + date[2];// + "/" + date[0];
                                    } else
                                    {
