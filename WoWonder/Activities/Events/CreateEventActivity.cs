            ...
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        var (apiStatus, respond) = await RequestsAsync.Event.CreateEventAsync(TxtEventName.Text, TxtLocation.Text, TxtDescription.Text, StartDate.Replace("/", "-"), EndDate.Replace("/", ""), TxtStartTime.Text.Replace("AM", "").Replace("PM", "").Replace(" ", ""), TxtEndTime.Text.Replace(" ", "-").Replace("AM", "").Replace("PM", ""), EventPathImage);
                        switch (apiStatus)
                        {
                            case 200:
                                {
        ...
        private void TxtAddOnClick(object sender, EventArgs e)
        {
            switch (nStep)
            {
                case 1: // Event name
                    ...
                    if (TxtEventName.Length() <= 10)
                    {
                        ToastUtils.ShowToast(this, GetText(Resource.String.Lbl_Please_enter_name_10), ToastLength.Short);
                        return;
                    }
                    break;
                ...
                case 4: // Event Photo
                    ...
                    break;
                case 5:
                    if (string.IsNullOrEmpty(TxtLocation.Text) || TxtLocation.Text.Equals(GetText(Resource.String.Lbl_Select_Location)))
                    {
                        ToastUtils.ShowToast(this, GetText(Resource.String.Lbl_Please_select_Location), ToastLength.Short);
                        return;
                    }
                ...
                case 6:
                    ...
                    {
                        ToastUtils.ShowToast(this, GetText(Resource.String.Lbl_Please_enter_Description_10), ToastLength.Short);
                        return;
                    }
                    break;
                default:
        ...
        public void OnClick(View v)
        {
            try
            {
                ...
                else if (v.Id == TxtStartDate.Id)
                {
                    ...
                    {
                        // ar dev
                        StartDate = time.Date.ToString("yy-MM-dd");
                        TxtStartDate.Text = time.Date.ToString("MM/dd/yy");
                ...
                else if (v.Id == TxtEndDate.Id)
                {
                    var frag = PopupDialogController.DatePickerFragment.NewInstance(delegate (DateTime time)
                    {
                        EndDate = time.Date.ToString("yy-MM-dd");
                        TxtEndDate.Text = time.Date.ToString("MM/dd/yy");
