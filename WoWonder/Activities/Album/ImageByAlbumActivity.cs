...
namespace WoWonder.Activities.Album
{
    ...
    public class ImageByAlbumActivity : BaseActivity
    {
            ...
            switch (item.ItemId)
            {
                case Android.Resource.Id.Home:
                    if (isDeleted)
                    {
                        ...
                        returnIntent?.PutExtra("DeleteAlbumPhoto", JsonConvert.SerializeObject(MAdapter.PhotosList));
            ...
            try
            {
                switch (addEvent)
                {
                    ...
                    case true:
                        ...
                        MAdapter.ItemCloseClick += MAdapterOnItelCloseClick;
                        break;
                    default:
                        MAdapter.ItemClick -= MAdapterOnItemClick;
                        ActionButton.Click -= ActionButtonOnClick;
        ...
        private void MAdapterOnItelCloseClick(object sender, PhotosAdapterClickEventArgs e)
        {
            try
            {
                PhotoAlbumObject item = MAdapter.GetItem(e.Position);
        ...
        private void DeletePhoto(PhotoAlbumObject item)
        {
            try
            {
                if (Methods.CheckConnectivity())
                {
                    ...
                    dialog.Title(GetText(Resource.String.Lbl_DeletePhoto)).TitleColorRes(Resource.Color.primary);
                    dialog.Content(GetText(Resource.String.Lbl_AreYouSureDeletePhoto));
                    dialog.PositiveText(GetText(Resource.String.Lbl_Yes)).OnPositive((materialDialog, action) =>
                    {
                        try
                        {
                            if (!Methods.CheckConnectivity())
                            {
                                ToastUtils.ShowToast(this, GetString(Resource.String.Lbl_CheckYourInternetConnection), Android.Widget.ToastLength.Short);
                                return;
                            }

                            MAdapter.PhotosList.Remove(item);
                ...
                {
                    ToastUtils.ShowToast(this, GetText(Resource.String.Lbl_CheckYourInternetConnection), Android.Widget.ToastLength.Short);
                }
            }
            catch (Exception exception)
            {
